...
export default {
  ...
  methods: {
      ...
      this.$confirm({
        ...
        buttons: [ // 按钮
          ...
          },
          {
            text: '确定',
            type: 'primary',
            callback: function () { // 点击确定触发
              // 准备请求参数
              let args = {
                merchantManagerId: row.id,
                merchantId: row.merchantId,
                signMarketingHumanId: row.signMarketingHumanId,
                signStartDate: row.signStartDate,
                signEndDate: row.signEndDate,
                performMarketingHumanId: row.performMarketingHumanId,
                performStartDate: row.performStartDate,
                performEndDate: row.performEndDate,
              }
              // 发送请求
              that.API.send(that.CFG.services.crmapi.merchantManagerCancelApplyAdd,  args, function () {
                that.$toast({text: '操作成功'})
                that.getData(that.searchParams);
                that.closeSelectResult();
              }, function () {
              }, that)
            }
          }
    ...
    reset() {
      ...
      // 重置搜索参数
      that.searchParams = {
        current: 1,// 当前页码，默认为第1页
        rowCount: 15,// 每页显示的行数，默认为15行
        // 商户名称
        merchantName: '',
        // 签约业务员姓名
        signMarketingName: '',
    ...
    choice() {
      const that = this;
      Object.assign(that.$data.generalSelectionModel, that.$options.data.call(that).generalSelectionModel);
      that.generalSelectionModel = {
        show: true,
        setting: that.commodityHousekeeperSetting,
      };
      const cols = that.generalSelectionModel.setting.cols.filter((item) => {
      ...
      cols.push({
        ...
        btns: [{
          ...
          click: (row) => {
              ...
              buttons: [ // 按钮
                ...
                },
                {
                  ...
                  callback: function () { // 点击确定触发
                    ...
                    that.API.send(that.CFG.services.crmapi.merchantManagerCancelApplyAdd,  args, function () {
                      that.$toast({text: '操作成功'})
                      that.generalSelectionModel.show = false;
                      that.getData(that.searchParams);
                    }, function () {
                    }, that)
                  }
            ...
            // this.$router.push({
            //   path: '/admin/ogHealthAgreementHousekeeperCancelAdd',
            //   query: {
            //     id: row.id
            //   }
            // });
      ...
      that.generalSelectionModel.setting.cols = cols;
    },
    /**
     * 更新当前页码。
    ...
    getData(args) {
      ...
      //发请求
      that.API.send(
        that.CFG.services.crmapi.merchantManagerCancelApplyPage,
        args,
        function(res) {
    ...
    toDel(row) {
      const that = this;
      let url = that.CFG.services.crmapi.merchantManagerCancelApplyDel;
    ...
    toEdit(row) {
      // 编辑页面 路由
      const path = '/admin/ogHealthAgreementHousekeeperCancelEdit';
      // 跳转路由
      this.$router.push({
        path: path,
        query: {
    ...
    toExamine(row) {
      // 审核页面 路由
      let path = '/admin/ogHealthAgreementHousekeeperCancelExamine';
      // 跳转路由
      this.$router.push({
        path: path,
        query: {
    ...
    //           //确定之后发请求
    //           that.API.send(
    //             that.CFG.services.crmapi.merchantManagerCancelApplyAudit, // 新url
    //             args,
    //             function (res) { //成功的回调函数
    //               that.$toast({ text: res.serviceResult || "操作成功" });
    //               that.updateCurrentPage();
    //               that.check.show = false;
    //             },
    //             function () {},//失败的回调函数
    //             that
    //           );
    ...
    toDetails(row) {
      // 详情页面 路由
      let path = '/admin/merchantButlerCancelDetail';
      // 跳转路由
      this.$router.push({
        path: path,
        query: {
    ...
    toAudit(row) {
      this.checkData = row;
      this.checkUrl = this.CFG.services.crmapi.merchantManagerCancelApplyAudit;
