        ...
        <div class="flex-flex-auto panel p15" ref="viewBody">
          <div class="panel-table-content">
            <el-table
              ...
              -->
              <el-table-column
                prop="partySubjectType"
                label="结算方类型"
                min-width="120"
                align='center'
                :resizable="false">
                <template slot-scope="scope">
                    <span v-if="scope.row.partyType=='1'">甲方</span>
                    <span v-else-if="scope.row.partyType=='2'">乙方</span>
                    <span v-else-if="scope.row.partyType=='3'">丙方</span>
                    <span v-else-if="scope.row.partyType=='4'">丁方</span>
                    <span v-else>--</span>
                </template>
              </el-table-column>
              <!-- 
              ...
              <el-table-column
                ...
                :resizable="false">
                <template slot-scope="scope">
                  <span v-if="scope.row.partySubjectType=='0'">业务组织</span>
                    <span v-else-if="scope.row.partySubjectType=='1'">商户</span>
                    <span v-else-if="scope.row.partySubjectType=='2'">业务员</span>
                    <span v-else-if="scope.row.partySubjectType=='3'">店铺</span>
                    <span v-else-if="scope.row.partySubjectType=='4'">代理（商户）</span>
                    <span v-else-if="scope.row.partySubjectType=='5'">自然人</span>
                    <span v-else-if="scope.row.partySubjectType=='6'">机构</span>
                    <span v-else>--</span>
                </template>
              </el-table-column>
              <!-- 
              ...
              <el-table-column
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.partyName| formatEmpty('--')}}
                </template>
              </el-table-column>
              <!-- 
                业务员级别
              ...
              <el-table-column
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.ootName | formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="ootSpecification"
                label="交易标的规格"
                min-width="120"
                align='center'
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.ootSpecification | formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="ootPackingSpecification"
                label="交易标的包装规格"
                min-width="150"
                align='center'
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.ootPackingSpecification | formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="ootMainUnit"
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.ootMainUnit | formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="ootManufacturerValue"
                label="生产厂家"
                min-width="120"
                align='center'
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.ootManufacturerValue | formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="ootMahValue"
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.ootMahValue | formatEmpty('--')}}
                </template>
              </el-table-column>
              <!-- 
                销售类型
              ...
              <el-table-column
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.businessType| formatEmpty('--')}}
                </template>
              </el-table-column>
              <!-- 
                价格类型
              ...
              <el-table-column
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.priceType| formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="calcWay"
              ...
              <el-table-column
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.fixedPrice | formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="benchPriceType"
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.benchPriceType | formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="ratio"
              ...
              <el-table-column
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.invalidTime | formatDate('yyyy-MM-dd hh:mm:ss')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="buyerRule"
                ...
                :resizable="false">
                <template slot-scope="scope">
                  {{scope.row.buyerRule | formatEmpty('--')}}
                </template>
              </el-table-column>
              <el-table-column
                prop="buyerAuth"
                ...
                :resizable="false">
                <template slot-scope="scope">
                  <!-- <span v-if="scope.row.buyerAuth === '1'">已认证</span>
                  <span v-else-if="scope.row.buyerAuth === '0'">未认证</span> -->
                  <!-- 
                    
                  -->
                  <!-- {{scope.row.buyerAuthList}} -->
                  <el-select v-model="scope.row.buyerAuthList"
                    class="full-width power-minw90" size="small"
                    disabled
                    multiple
                    placeholder="无数据">
                    <el-option v-for="item in limitAuthTypeOpts" :key="item.code" :label="item.name" :value='item.code'></el-option>
                  </el-select>
                </template>
              </el-table-column>
              <el-table-column
                ...
                :resizable="false">
                <template slot-scope="scope">
                  <span v-if="scope.row.partyBType === '1'">自然人</span>
                  <span v-else-if="scope.row.partyBType === '2'">组织</span>
                </template>
              </el-table-column>
              <el-table-column
                prop="version"
...
export default {
  ...
  methods: {
    ...
    getData (data) {
      ...
      this.API.send(this.CFG.services.ca.getAllCode, args, function (resA) {
        resA = resA.serviceResult || [];
        this.limitAuthTypeOpts = resA;
        this.listEnd = [];
        this.API.send(this.CFG.services.settlementNew.queryPagedPriceResultList, data, function (res) {
          res = res.serviceResult;
          this.list = res.rows || [];
          let id = '';
          if(this.list.length){
            this.list.forEach(item => {
              // item.levelName = this.levelName(item.level);
              id = id + ',' + item.ootId;
            })
            this.API.send(
              this.CFG.services.settlement.openQueryObjects, {ids: id}, function (resA) {
                resA = resA.serviceResult;
                this.list.forEach(item => {
                  if(resA[item.ootId]){
                    // delete resA[item.ootId].id;
                    // item = this.utils.extend({},item, resA[item.ootId]);
                    item = this.utils.extend({},item, {ootName: resA[item.ootId].name, ootSpecification: resA[item.ootId].specification, ootPackingSpecification: resA[item.ootId].packingSpecification, ootMainUnit: resA[item.ootId].mainUnit, ootManufacturerValue: resA[item.ootId].manufacturerValue, ootMahValue: resA[item.ootId].mahValue});
                    item.authList = resA;
            ...
            console.info('1111', this.listEnd);
          } else {
            this.listEnd = [];
          }
