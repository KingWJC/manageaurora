<template>
  <div class="viewport-fixed">
    ...
    <div class="viewport-view">
      <div class="viewport-view-body flex flex-column">
        <div class="panel-content-padded">
          <h2 class="pl15 pb10">协议嵌套关联关系解除申请单</h2>
          <div class="pl15 pr15">
                      ...
                      <div class="card card-shadow mt15 lc-col-12 lc-col-xs12" v-for="item in list"
                        ...
                        <div class="card-body">
                          ...
                          <div class="card-footer">
                            <span class="black">是否要解除此协议嵌套关联关系:</span>
    ...
    <el-dialog title="解除" :modal-append-to-body="false" :close-on-click-modal="false"
  ...
  methods: {
    ...
    toEditEntter (){
      ...
      if (!this.checkData.effectType) {
       ...
       if (this.checkData.effectType === '2' && !this.checkData.effectTime) {
        this.$toast({ text: `请选择生效时间` });
        return false
      }
      this.$confirm({
        title: '提示',
        text: '确定要解除此协议嵌套关联关系吗？解除后不可恢复！',
        buttons: [
          {
            text: '取消',
            type: 'follow'
          },
          {
            ...
            callback: function () {
              ...
              that.API.send(that.CFG.services.ecRef.viewById, {id: that.checkData.contractId}, function (res) {
                res = res.serviceResult;
                res.type = '5';
                res.refId = res.id;
                delete res.id;
                res.effectType = this.checkData.effectType
                if(this.checkData.effectTime){
                  res.effectTime = this.checkData.effectTime
                }
                that.API.send(that.CFG.services.ecRefApply.update, res, function () {
