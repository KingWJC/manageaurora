            ...
            <div class="panel-table-content">
              ...
              >
                ...
                <el-table-column prop="hjje" label="合计金额" min-width="120">
                  <template slot-scope="scope"
                    >¥{{ Number(scope.row.hjje || 0).toFixed(2) }}</template
                  >
                </el-table-column>
                <el-table-column prop="hjse" label="合计税额" min-width="120">
                  <template slot-scope="scope"
                    >¥{{ Number(scope.row.hjse || 0).toFixed(2) }}</template
                  >
                </el-table-column>
                <el-table-column prop="jshj" label="价税合计" min-width="120">
                  <template slot-scope="scope"
                    >¥{{ Number(scope.row.jshj || 0).toFixed(2) }}</template
                  >
                </el-table-column>
                <el-table-column
                  prop="kpr"
  ...
  methods: {
    ...
    buildQueryPayload() {
      ...
      if (Array.isArray(kprqRange) && kprqRange.length === 2) {
        // 转换为 ISO 8601 格式
        payload.kprqStart = kprqRange[0]
          ? new Date(kprqRange[0] + 'T00:00:00.000Z').toISOString()
          : undefined;
        payload.kprqEnd = kprqRange[1]
          ? new Date(kprqRange[1] + 'T23:59:59.999Z').toISOString()
          : undefined;
      }
      return payload;
    },
    ...
    toCreate() {
      this.$router.push({ name: 'taxInvoiceBlueDetail' });
    },
    statusText(v) {
      if (v === '00') return '未提交';
      if (v === '01') return '开票中';
