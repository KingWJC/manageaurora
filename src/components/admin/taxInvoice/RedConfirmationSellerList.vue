        ...
        },
        {
          ...
          btns: [
            ...
            },
            {
              ...
              condition: () => {
                return this.permissions.names.invoice;
              },
              isDisabled: (row) => {
  ...
  methods: {
    ...
    buildQueryPayload() {
      ...
      if (Array.isArray(lrrqRange) && lrrqRange.length === 2) {
        if (lrrqStart !== undefined)
          payload.lrrqStart = String(
            new Date(lrrqRange[0] + 'T00:00:00').getTime()
          );
        if (lrrqEnd !== undefined)
          payload.lrrqEnd = String(
            new Date(lrrqRange[1] + 'T23:59:59').getTime()
          );
      }
      return payload;
    },
    ...
    canInvoice(row) {
      ...
      return (
        (row.hzqrxxztDm === '01' || row.hzqrxxztDm === '04') &&
        !row.redInvoiceId
      );
    },
    submitRow(row) {
    ...
    parsePagedResult(payload = {}) {
      ...
      const successFlag =
        serviceResult && serviceResult.success !== undefined
