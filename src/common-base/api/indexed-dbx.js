...
IndexedDBX.prototype.initStore = function (database, success, error, context = this) {
  let tables = this.settings.dataBase.tables || [];
  try {
    for (let table of tables) {
      // 如果没有创建表,则创建表
      if (!database.objectStoreNames.contains(table.name)) {
        let store = database.createObjectStore(table.name, {keyPath: table.keyPath});
        // 创建索引
...
IndexedDBX.prototype.openDatabase = function (success, error, context = this) {
  ...
  if (this.indexedDB) {
    try {
      let request = this.indexedDB.open(this.settings.dataBase.dbName, this.settings.dataBase.version);
      ...
      request.onupgradeneeded = function (event) {
        ...
        that.initStore(database,
          function () {
            // *执行成功后，会触发request.onsuccess，这里无需处理
            //if (typeof success === 'function') {
            //  success.call(context, database);
            //}
          },
...
IndexedDBX.prototype.addOrUpdate = function (storeName, data, success, error, context = this) {
  ...
  try {
    this.openDatabase(
      function (database) {
        let store = this.getStore(database, storeName, 'readwrite');
      ...
      function (err) {
        console.log('addOrUpdate openDatabase err', err);
      },
      this
    );
  } catch (e) {
...
IndexedDBX.prototype.get = function (storeName, params, success, error, context = this) {
  ...
  this.openDatabase(
    function (database) {
      let store = this.getStore(database, storeName, 'readonly');
      let request;
      if (params.keyPath) {
...
IndexedDBX.prototype.matchedData = function (arr, prop, keyword, isExact = true) {
  ...
  arr.forEach(function (item) {
    if (regExp.test(item[prop])) {
      ...
      // 不匹配本级，检查是否匹配子
      let matchedChildren = this.matchedData(item.children, prop, keyword, isExact);
