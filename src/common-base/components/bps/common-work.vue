 ...
 * @LastEditTime: 2023-09-18 16:37:35
-->
<template>
  <div class="card card-shadow bps-container mb35">
    <div class="viewport-footer viewport-footer-padded flex flex-column flex-content-center pb10">
        <!-- 底部按钮 -->
        <div class="flex flex-content-center" >
          ...
          <template v-if="params.bpsType === 'approve'">
            ...
            <button @click="showOperate(btn)" v-for="btn in oprTypes" :key="btn" class="btn btn-xl mr20 inline-block" :class="btn === 103 ? 'btn-primary': '' " native-type="submit" >{{getOprTypesWorkName(btn)}}</button>
            </div>
          </template>
        </div>
    </div>
    <div class="card-body" v-if="params.bpsType === 'approve'">
      <div class="card-header">
        <h4 class="inline">处理节点</h4>
        <span class="blue" v-if="params.bpsType === 'approve' && processInfo && processInfo.state !== 100">({{getWorkStateName(processInfo.state)}})</span>
      </div>
      <!-- 发起人审批模版 -->
    ...
    <el-dialog
      :title="getOprTypesWorkName(approveForm.oprType)"
...
import { getApprovalWayType, tmpNodeDefaut, commonUrl, getOprTypesWorkName, getWorkStateName, approveForm } from "./components/dit.js"
import { selectHumanLeaderslog, selectHumanLeaderlog,  arrToStr, getOrgType, getWorkRollbackNodeList, getWorkNextNodeList, getUsersInfo } from "./components/utils.js"
...
export default {
  ...
  mounted() {
    if(this.params.bpsType === 'approve'){
  ...
  methods: {
    ...
    initFormConfig(){
      this.formConfig = [
        ...
        // },
        {
          label: '加签人', id: 'addUserIdNames', required: true, type: 'text-btn', disabled: true, showAdd: true, mWidth: '100px', rows: 2, maxlength: 500, placeholder: '请添加加签人', lg: 24, xl: 24,md: 24, model: this.approveForm,
          hide: ()=>{
        ...
        {
          label: '下一步节点', id: 'nextToNodeId', required: true, type: 'select', mWidth: '100px', maxlength: 500, placeholder: '请选择下一步节点', lg: 24, xl: 24,md: 24, model: this.approveForm,
          hide: ()=>{
            return this.approveForm.oprType !== 109
          },
          change: (val, conf, from)=>{
            from.nextToNodeName = conf.options.find(v=> v.c === val) ? conf.options.find(v=> v.c === val).n : '';
          },
          options: []
        },
        {
    ...
    async showOperate(oprType){
      ...
      if(oprType === 107){
        ...
        this.approveForm.addUserIdNames = '';
        this.approveForm.addUserIds = [];
      }
      if(oprType === 104){
        let options = await getWorkRollbackNodeList.bind(this, this.processInfo.id)();
        if(!options.length){
          this.$message({
            type: 'info',
            message: '暂不存在回退节点',
        ...
        if(options.length === 1){
          this.approveForm.rollbackToNodeId = options[0].nodeId;
          this.approveForm.rollbackToNodeName = options[0].nodeName;
        }
      }
      if(oprType === 109){
        let options = await getWorkNextNodeList.bind(this, this.processInfo.id)();
        if(!options.length){
          this.$message({
            type: 'info',
            message: '暂不存在下一步节点',
        ...
        if(options.length === 1){
          this.approveForm.nextToNodeId = options[0].nodeId;
          this.approveForm.nextToNodeName = options[0].nodeName;
        }
      }
    ...
    submitOperate(){
      ...
      this.API.send(commonUrl.workOrderOperate, args, (res)=> {
        res = res.serviceResult;
    ...
    // 允许添加人范围
     canAddUserRange(){
      ...
      this.API.send(commonUrl.canAddUserRange, args, async (res)=> {
        res = res.serviceResult;
        if(res.userIds){
          let users = await getUsersInfo.bind(this, { ids: res.userIds })();
          this.checkedUserList = users;
        }
        this.checkedOrgList = res.orgIds;
        this.activeNode.orgType = res.orgType;
        this.scopeType = 1;
      }, ()=> {
      }, this)
    },
    ...
    async sureApprover(data){
      ...
      if(this.approveForm.oprType === 107){
        this.approveForm.addUserIds.push(...ids);
        this.approveForm.addUserIdNames = arrToStr(data);
      }
    ...
    bpsCommitCallback(res){
      ...
      if(res){
        let args = {
          bizProcessId: this.params.bpsId,
          formInstanceId: res
        }
        this.submit(args);
      }else {
    ...
    // 发起提交审核
    submit(args){
      this.API.send(commonUrl.workOrderSubmit, args, function (res) {
        res = res.serviceResult;
        console.log(res);
        this.$router.go(-1);
        // LonchJsApi.openUrl({
        //   originName: 'manage',
        //   routePath:'/admin/careerPortfolio/detail',
        //   query: {
        //     moduleCode: '196008',
        //   }
        // })
